...
tensor = np.ones((len(x),len(y),len(z),9,3))
...
for item in directories:
    ...
    for file in directory:
        if file.startswith('log-'):
            ...
            tensor[tuple(e_position)] = data
        np.save(f'/home/davialefe/docking/cov/main_protease/tensors_all_data/{item}',tensor)