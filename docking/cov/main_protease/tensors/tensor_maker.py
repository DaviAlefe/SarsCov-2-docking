...
tensor = np.ones((len(x),len(y),len(z)))
...
for item in directories:
    ...
    for file in directory:
        if file.startswith('log-'):
            ...
            energy = tools.extract_affinity(path)
            ...
            tensor[tuple(e_position)] = energy
        np.save(f'/home/davialefe/docking/cov/main_protease/tensors/{item}',tensor)